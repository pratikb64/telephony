	...
	<div>
		...
		>
			...
			<div
				...
				class="blink"
				:class="{
					'text-red-700': callStatus == 'Call ended' || callStatus == 'No answer',
				}"
			>
		...
		<div
			...
			class="fixed z-20 w-[320px] min-h-44 flex gap-2 flex-col rounded-lg bg-surface-gray-7 p-4 pt-2.5 text-ink-gray-2 shadow-2xl"
			:style="style"
			@click.stop
		>
			<div
			...
			>
				<div class="flex gap-2 items-center truncate">
					<div v-if="showNote || showTask" class="flex items-center gap-3 truncate">
						<Avatar
							v-if="contact?.image"
							:image="contact.image"
							:label="contact.full_name"
						...
						<div class="flex flex-col gap-1 text-base leading-4 overflow-hidden">
			...
			<div class="body flex-1">
				...
				<div v-else class="flex items-center gap-3">
					...
					<div v-if="contact?.full_name" class="flex flex-col gap-1">
						<div class="text-lg font-medium leading-5 text-ink-white">
							{{ contact.full_name }}
						</div>
						<div class="text-base text-ink-gray-5 leading-4">
							{{ contact.mobile_no }}
						</div>
					</div>
...
function updateStatus(data) {
	...
	if (
		...
		callDuration.value = counterUp.value.getTime(
			parseInt(data["Legs[0][OnCallDuration]"]) || parseInt(data.DialCallDuration),
		);
		return "Call ended";
	}
